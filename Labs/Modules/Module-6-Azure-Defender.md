...
1. Create an alert simulation for VM:
    ...
    - Click on the **Create sample alerts** button to trigger the alert simulation for VMs.
        ...
        ![](../Images/hyb-ex4-g37.png)

1. You will see sample alert creation in progress; wait for the process to complete. Now you can track the progress by opening the notification center or on the activity log (this process usually takes 2 minutes to complete)

1. On the **Microsoft Defender for Cloud | Security alerts** page, you should now see 5 different sample events for a resource named `Sample-VM`. Each alert has the Sample alert banner next to it.

    ![](../Images/m6ex1step5.png)

1. Click on the **Digital currency mining related behavior detected** alert.

1. After the details pane opens, notice the alert properties at the top (severity, status, and time) along with the alert's description and affected resources. At the bottom section, you can see the kill chain intent of the alert based on the MITRE ATT&CK® framework. This specific alert is in the execution phase.
...
1. Create an alert simulation for Key Vaults:
    ...
    - Click the **Create sample alerts** button to trigger the alert simulation for Key Vaults.
        ...
        ![](../Images/hyb-ex5-g6.png)
    
1. After the Sample alerts creating in progress appear, wait for the process to complete. You can track the progress by opening the notification center or on the activity log (this process usually takes 2 minutes to complete)

     ![add](../Images/hyb-ex4-g38.png)
...
1. Select **Access from a TOR exit node to a Key Vault** alert then click on the **View full details** to see additional information related to the event.
     ...
     ![Create sample virtual machine security alerts](../Images/hyb-ex5-g7.png)

1. At the top menu, dismiss the alert by changing the status from Active to **Dismissed** and click on **OK**.

     ![add](../Images/hyb-ex5-g8.png)
...
1. Select **High volume of operations in a Key Vault** alert and then click on **Take action**.

    ![Create sample virtual machine security alerts](../Images/m6ex2step2.png)

1. Expand the **Suppress similar alerts** under the Take action section and click on **Create Suppression Rule**.

    ![](../Images/hyb-ex5-g9.png)

1. After the **New suppression rule** pane opens, enter the following details:
   ...
   - Click on **Apply (6)** and wait 10 minutes for the new rule to be applied.
        ...
        ![Create sample virtual machine security alerts](../Images/hyb-ex5-g11.png)
...
1. You can change your existing suppression rules or create new ones: On the **Security alerts** page from the top menu, select **Suppression rules**. 

    ![Create sample virtual machine security alerts](../Images/hyb-ex5-g12.png)

1. Click on the rule you have recently created: `Testing-AutoDismiss-KV` and change the expiration to a month ahead of the current date. Next, click on **Apply**.

    ![](../Images/hyb-ex5-g13.png)
  
    > **Note**: If you receive any pop-up stating the **Warning** alert, click on **OK**

1. To view the dismissed alerts: Navigate to Security Alerts on the main page and to the filters section. Change the **Status (1)** filter value to show only **Dismissed (2)** items and click **Ok (3)**.
...
1. Scroll down to the **Example** section for a request or alternatively you can open this URL: https://docs.microsoft.com/en-us/graph/api/alert-list?view=graph-rest-1.0&tabs=http#example in your **labvm-xxxxxx** browser window.

1. On the HTTP tab, review the endpoint URL: `https://graph.microsoft.com/v1.0/security/alerts`.

1. Click on **Try It (1)**, then click on the **User icon (2)** to login. You need to log in using the credentials provided in the **Environment Details** tab.

    ![Create sample virtual machine security alerts](../Images/hyb-ex5-g14.png)

    ![Create sample virtual machine security alerts](../Images/hyb-ex5-g15.png)

    ![Create sample virtual machine security alerts](../Images/hyb-ex5-g16.png)

1. Click on **Accept** to allow access.

    ![Consent-Accept](../Images/hyb-ex5-g17.png)

1. Now click on **Modify permissions** and wait for two permissions to load. Scroll to the right side of permissions, then click on **Consent**, do this for both permissions. When prompted for login, login using credentials provided in the **Environment Details** tab, click on **Accept** for each permission to get **Consented**

   > **Note**: If you are not able to get **Consented**, please resize the browser window.
 
    ![Consent](../Images/hyb-ex5-g18.png)

    ![Consent](../Images/hyb-ex5-g19.png)

1. Click Accept.

    ![Consent2](../Images/hyb-ex5-g20.png)
    
1. Replace the endpoint URL with the following endpoint URL:`https://graph.microsoft.com/v1.0/security/alerts?$top=5` **(1)**. This query returns a list of the top five security alerts using the Graph Security API.

    ![Create sample virtual machine security alerts](../Images/hyb-ex5-g21.png)    

1. Click on the **Run query** button. You should now see a sample response preview in JSON format.
